...
export function ChatInterface() {
    ...
    try {
        ...
        mediaRecorderRef.current.onstop = async () => {
            ...
            try {
              ...
              const res = await getVoiceResponse({ audioDataUri, previousContext: chatHistory });
  ...
  const handleSubmit = async (e: React.FormEvent, mode: 'text' | 'image') => {
    e.preventDefault();
    if (isLoading || (mode === 'text' && !input.trim()) || (mode === 'image' && !image)) return;

    let userMessage: ChatMessageType;
    if (mode === 'text') {
        userMessage = { id: Date.now().toString(), role: 'user', content: input };
    } else if (image) {
        userMessage = { 
    ...
    try {
        ...
        if (mode === 'text') {
            res = await getTextResponse({ query: userMessage.content as string, chatHistory });
        } else if (image) {
            const imageDataUri = await fileToDataUri(image.file);
            res = await getImageResponse({ question: input, imageDataUri, chatHistory });
        }
